# Requirements Archive: v1.1 Data-Quality Hardening
...
- [x] **CONF-06**: 각 로펌은 선택적 `include_keywords` / `exclude_keywords` 필터를 지원한다
...
- [x] **DQOBS-01**: The GHA step-summary table adds per-firm columns — average body length, generic-body count (GUARD triggers), confidence distribution (high/medium/low)
- [x] **DQOBS-02**: If a firm's most recent run produces ≥50% `confidence: 'low'` summaries, the email footer and step-summary flag it as a data-quality concern
- [x] **DQOBS-03**: `DRY_RUN=1` mode prints the same DQOBS metrics without writing state or sending email, so the operator can diagnose before unpausing cron

### Cron Resumption Gate (RESUME)

- [x] **RESUME-01**: After all v1.1 phases complete, one manual `workflow_dispatch` run is executed and its digest is visually inspected for hallucinated-summary regressions
- [x] **RESUME-02**: `.github/workflows/daily.yml` `schedule:` is uncommented (cron restored) only after RESUME-01 passes and a dated acceptance note is recorded in STATE.md
