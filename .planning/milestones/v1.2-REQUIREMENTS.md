# Requirements Archive: v1.2 Coverage & Closure
...
- [x] **CLOSURE-01**: `.planning/milestones/v1.1-phases/10-data-quality-observability/10-VERIFICATION.md` exists with `status: passed` (or `gaps_found` with documented gaps), backfilled goal-backward against DQOBS-01/02/03 evidence (including the post-v1.1-close fix at commit 04a572e for DQOBS-01). Phase dirs may have been archived to `milestones/v1.1-phases/` by this point — write the verification at whichever path the phase currently lives. **Shipped:** Phase 15 / commit `de76809` (`.planning/phases/10-data-quality-observability/10-VERIFICATION.md`, `status: passed`).

- [x] **CLOSURE-02**: Phase 11 (Cron Resumption Gate) closure artifacts complete: (a) `11-03-SUMMARY.md` written documenting the actual cron uncomment outcome with commit references (cron is uncommented in `.github/workflows/daily.yml:27` + `weekly.yml:30`), and (b) `11-VERIFICATION.md` written with `status: passed`, evidence against RESUME-01 (smoke test 2026-04-21 + production verification 2026-05-23 workflow_dispatch runs 26335283814 + 26335329895) and RESUME-02 (cron lines factually uncommented). **Shipped:** Phase 15 / commit `cc5468a` (`11-03-SUMMARY.md` + `11-VERIFICATION.md`, atomic).
...
|-------------|-------|--------|----------|
| SCHED-01 | Phase 14 — Scheduling Coverage | Complete (code) / human_needed (time-delayed Mon cron audit on 2026-06-03) | `.github/workflows/daily.yml:28` cron `0 12 * * 0-6` + `weekly.yml:30` cron `0 21 * * 0`; workflow_dispatch run `26513050608` + `26513052641` confirm parser acceptance |
| SCHED-02 | Phase 14 — Scheduling Coverage | Complete | `pnpm sync-schedule` removed (package.json + `src/config/loader.ts` `toCron` deleted); CLAUDE.md `## Conventions > Workflow scheduling` policy added |
| CLOSURE-01 | Phase 15 — v1.1 Phase Closure Backfills | Complete | `.planning/phases/10-data-quality-observability/10-VERIFICATION.md` status: passed (commit `de76809`); DQOBS-01/02/03 + `04a572e`/`260523-mtz` cited |
| CLOSURE-02 | Phase 15 — v1.1 Phase Closure Backfills | Complete | `11-03-SUMMARY.md` + `11-VERIFICATION.md` (commit `cc5468a`, atomic); RESUME-01/02 + GHA runs `26335283814`/`26335329895` + `daily.yml:27`/`weekly.yml:30` cited |
| META-01 | Phase 16 — v1.1 Metadata Hygiene | Complete | `.planning/milestones/v1.1-REQUIREMENTS.md` traceability table 4-column with 6 REQ flips (commit `b8acd4a`); Phase 10/11/12 SUMMARYs `requirements-completed` frontmatter backfilled (commit `38862b2`) |
| META-02 | Phase 16 — v1.1 Metadata Hygiene | Complete | `06-AUDIT.md` regenerated via `pnpm audit:firms` (commit `c517cc3`); CLAUDE.md `## Conventions > Audit freshness` subsection added |
| FAIL-UX-01 | Phase 17 — Summary Failure UX Cleanup | Complete | `renderArticle()` failed branch (commit `1cda441`); `parseRetryDelaySeconds` + async `onFailedAttempt` sleep cap 60s (commit `14a0e28`); CLAUDE.md flash RPM `5 (observed 2026-05-27)` + quota-pool note (commit `24deed1`); 495/495 tests pass |
