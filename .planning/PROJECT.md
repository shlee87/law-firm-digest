...
- **v1.2 Coverage & Closure** — Phases 14–17 (shipped 2026-05-28, archived to `milestones/v1.2-*`). 10 plans across 4 phases. Monday fetch gap closed; sync-schedule footgun eliminated; v1.1 archive metadata reconciled (CLOSURE-01/02 + META-01/02); Gemini summary-failure UX cleaned up + retryDelay-aware p-retry. 495/495 tests green. Time-delayed Monday-cron audit deferred to 2026-06-03 (pre-staged in 14-VERIFICATION.md).

## Next Milestone

📋 **v1.3** — Awaiting `/gsd:new-milestone` (questioning → research → requirements → roadmap). No commitments yet — natural next directions surfaced during v1.2: production observability dashboard, multi-recipient support, summary quality scoring, but all explicitly deferred. Operator picks scope at milestone-open.
...
- ✓ 클라우드 자동 실행 — GitHub Actions cron (daily.yml Mon–Sun 21:00 KST + weekly.yml Mon 06:00 KST) — v1.0 + v1.1 + v1.2 (Phase 14 Monday backfill + sync-schedule footgun 제거)
- ✓ $0 운영 비용 유지 (Gemini 무료 티어 + GHA 무료 분 + Gmail SMTP App Password) — v1.0 (유지 중)
- ✓ **SCHED-01/02 (Phase 14):** Monday daily fetch backfill (cron `0 12 * * 0-6`) + weekly time-separated to Mon 06 KST; `pnpm sync-schedule` 영구 제거 + CLAUDE.md 단일-진실-원천 정책 — v1.2
- ✓ **CLOSURE-01/02 (Phase 15):** v1.1 Phase 10/11 VERIFICATION.md + 11-03-SUMMARY.md backfilled goal-backward against shipped code (DQOBS-01/02/03 + RESUME-01/02 evidence pinned) — v1.2
- ✓ **META-01/02 (Phase 16):** v1.1 traceability 4-column with 6 REQ flips + Phase 10/11/12 SUMMARY `requirements-completed` frontmatter + 06-AUDIT.md regenerated + CLAUDE.md audit freshness policy — v1.2
- ✓ **FAIL-UX-01 (Phase 17):** Gemini summary failure no longer leaks title duplication or raw error JSON to recipient HTML; `parseRetryDelaySeconds` + async `onFailedAttempt` sleep honors 429 retryDelay; CLAUDE.md free-tier table updated (flash `5 RPM observed 2026-05-27`, flash+lite quota pool shared) — v1.2

### Active

<!-- Current scope. Building toward these. -->

v1.2 closed 2026-05-28. All v1.2 requirements shipped and moved to Validated. No active requirements until `/gsd:new-milestone` opens v1.3.

**Outstanding post-deploy task:** Phase 14 SC #1 — Monday cron observability audit on or after 2026-06-03 (command pre-staged in `.planning/phases/14-scheduling-coverage/14-VERIFICATION.md` `human_verification:` block). Not blocking v1.2 close per SPEC §Out-of-scope + CONTEXT D-04.
...
| Cron edit policy: yml 직접 편집 (sync-schedule 폐기) | `pnpm sync-schedule` 가 Phase 13 split을 silently overwrite하던 footgun. CLAUDE.md `## Conventions > Workflow scheduling` 에 단일 진실 원천(daily.yml + weekly.yml의 cron 라인) + 편집 절차 명문화 | ✓ Good (v1.2 Phase 14) |
| Time-delayed audit deferral pattern | Phase 14 SC #1 (1주일 Mon cron observability)는 code merge 시점에 검증 불가 — `human_verification:` block + STATE.md 앵커로 미래 audit 명령 사전 등록 | ✓ Good (v1.2 Phase 14; 2026-06-03 audit window) |
| Failed Gemini summary: body omit + mini-tag | demoted block 강등 대신 renderArticle에 failed 분기 추가 — 3-tier failure semantics(skipped/failed/demoted)의 시각적 구분 보존 + summaryError를 user-facing HTML에 절대 노출 안 함 | ✓ Good (v1.2 Phase 17) |
| Gemini 429 retryDelay honor | flash + flash-lite가 같은 quota metric 공유 — 모델 폴백만으론 429 unstick 불가. p-retry `onFailedAttempt` 가 async로 retryDelay sleep(60s cap) → 다음 분 RPM window까지 실제 대기 | ✓ Good (v1.2 Phase 17; mock 429 with retryDelay=30s 검증) |
